...
import { getToken, hostFetch, getVerifyJwk } from "./client_v-7HzW5v.mjs";
import { r as randomId, v as verifyEnvelope, i as importVerifyKey, p as payloadOf } from "./stripe_r-RFTlbb.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { p as preflight, i as isRunnableDefinition } from "./preflight_B1R0Zxz-.mjs";
import { sanitizeRenderHtml, validateDefinition, isBlockedHost } from "./appdef_DMKxCl1j.mjs";
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_DMKxCl1j.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_DENZRpVO.mjs");
