...
import { setMaxUploadMb, setRetentionDays } from "./storage_4iye1Zw4.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_TF_RyozG.mjs";
import { setStorageLimits } from "./storage-usage_B5o6RDLs.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_CnHsjCoE.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, i as installLocalApp, a as activeAppDefinition, h as appVersions, j as activateAppVersion, k as deleteLocalApp } from "./external-apps_H2ZAmhVv.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_DENZRpVO.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_sNsbonJs.mjs");
    let sent = false;
    let okHost = false;
    try {
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_H2ZAmhVv.mjs").then((n) => n.A);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_H2ZAmhVv.mjs").then((n) => n.z);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_H2ZAmhVv.mjs").then((n) => n.A);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_H2ZAmhVv.mjs").then((n) => n.A);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_H2ZAmhVv.mjs").then((n) => n.z);
