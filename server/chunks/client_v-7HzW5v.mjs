...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_Bv3SAITG.mjs").then((n) => n.S), import("./notifications_sNsbonJs.mjs")]);
    ...
    await (await import("./ctx_Bv3SAITG.mjs").then((n) => n.P)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_Bv3SAITG.mjs").then((n) => n.M)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_FzfO26ot.mjs");
